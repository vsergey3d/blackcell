...
function getAncestorList(doclet) {
    var ancestors = [],
        doc = doclet.memberof;
    ...
    while (doc) {
        ...
        if (doc) { doc = doc[0]; }
        if (!doc) { break; }
        ancestors.unshift((helper.scopeToPunc[doc.scope] || '') + doc.name);
        doc = doc.memberof;
    }
    if (ancestors.length) {
...
function getAncestorLinks( doclet ) {
	return helper.getAncestorLinks( data, doclet, "value" );
}
...
function getOnlySignatureParams(d) {
    var pnames = [];
...
function addSignatureParams( f ) {
    ...
    f.signatureParams = getOnlySignatureParams(f).join( ', ' );
}
...
function addSignatureReturns( f ) {
    ...
    f.signatureReturns = returnTypes.length ? returnTypes.join( '|' ) : '';
}

function addSignatureTypes( f ) {
	var types = helper.getSignatureTypes( f );

    f.signatureTypes = types.length ? ' :' + types.join( '|' ) : '';
    f.signature = (f.signature || '') + '<span class="type-signature">' + f.signatureTypes + '</span>';
}

function addAttribs( f ) {
	var attribs = helper.getAttribs( f );

    f.attribsStr = attribs.join( ', ' );
    f.attribs = '<span class="type-signature">' + htmlsafe( attribs.length ? '<' + f.attribsStr + '> ' : '' ) + '</span>';
}
...
function generate( docType, title, docs, filename, resolveLinks ) {
    ...
    var docData = {
		title   : navOptions.systemName + " - " + title,
		docs    : docs,
		docType : docType
	};
	...
	if ( resolveLinks ) {
		html = fixLinks(helper.resolveLinks( html )); // turn {@link foo} into <a href="foodoc.html">foo</a>
	}
...
exports.publish = function ( taffyData, opts, tutorials ) {
	...
	var fromDir = path.join( templatePath, '../' + conf.static ),
	...
	data().each( function ( doclet ) {
        ...
        doclet.ancestorList = getAncestorList(doclet);
	...
	for ( var longname in helper.longnameToUrl ) {
		if ( hasOwnProp.call( helper.longnameToUrl, longname ) ) {
			...
			if ( myNamespaces.length ) {
				generate( 'module', 'Module: ' + myNamespaces[0].name, myNamespaces, helper.longnameToUrl[longname] );
			}
